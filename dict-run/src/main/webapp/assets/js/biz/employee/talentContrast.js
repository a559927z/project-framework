 ...
 */
require(['jquery', 'moment', 'echarts', 'echarts/chart/line', 'bootstrap', 'utils', 'ace', 'jquery-pin', 'jgGrid', 'underscore'], function ($, moment, echarts) {
    var webRoot = G_WEB_ROOT, winObj = top != window ? top.window : window;
    var urls = {
        getAbilityUrl: webRoot + '/common/getAbility.do',               //获取职级信息
    ...
    var floatNumber = Tc.formatFloat(document.body.clientHeight / 3 - $('.menu').width() / 2, 0);
    $('.menu').css('top', floatNumber);
    var empIds = "", rowIds = {};
    ...
    var abilityObj = {
        ...
        requestData: function () {
            ...
            $.get(urls.getAbilityUrl, function (rs) {
                ...
                self.data = _.map(rs, _.property('name'));
            });
        }
    }
    abilityObj.init();
    ...
    var searchEmpObj = {
        ...
        gridOption: {
            ...
            colModel: [
                {name: 'empKey', width: 100, sortable: false, align: 'center'},
    ...
    var jobChangeChartOption = {
        tooltip: {
            ...
            formatter: function (params) {
                var obj = params[0].data.data;
                return '<div class=""><h5>' + params[0].name + '</h5>岗位：' + obj.positionName + '<br>职级：' + obj.rankName + '</div>';
            }
        },
        grid: {
            x: 50,
        ...
        yAxis: [
            ...
            },
            {
                ...
                splitLine: false,
                min: 1,
                max: _.isNull(abilityObj.data) ? 5 : abilityObj.data.length,
                splitNumber: _.isNull(abilityObj.data) ? 5 : abilityObj.data.length,
                axisLine: {
                    lineStyle: {
                        color: '#BCBCBC'
                    }
    ...
    var perfObj = {
        ...
        extendData: function (source, index, empId) {
            ...
            $.each(newSource, function (i, obj) {
                ...
                if (month > 1 && month <= 3 || month > 7 && month <= 9) bool = true;
                if (!chartData.xAxisYearData) chartData.xAxisYearData = [];
                if (!chartData.xAxisQuarterData) chartData.xAxisQuarterData = [];
                if (!chartData.seriesData) chartData.seriesData = [];
    ...
    function searchBtnEvent() {
        ...
        $('#search-btn').click(function () {
            ...
            if (!_.isEmpty(searchTxt)) searchEmpObj.init(searchTxt);
        });
    ...
    function removeOfCompareDiff(_obj, index) {
        ...
        $.each(_objSib, function (i, sib) {
            ...
            if (_objSibChild.length < 1) return true;
            var hasChild = 0;
            ...
            if (hasChild > maxNum) maxNum = hasChild;
        });
        if (maxNum > 0) {
    ...
    function addColClickEvent(e) {
        var _this = $(this), empId = _this.data('index'), rowId = _this.data('row');
        $.post(urls.getEmpInfoUrl, {'empIds': empId}, function (rs) {
            if (_.isEmpty(rs)) return;

            empIds += ',' + empId;
            rowIds[empId] = rowId;
            var empObj = rs[0], idx = $('#search-index').val();
    ...
    $(window).scroll(function (e) {
        if (!isCurPage) return false;
        var top = $(this).scrollTop(), $menu = $(".menu");
        //top-fa控制
        if (top >= 25) {
            ...
            if ($(".top-fa").hasClass("top-fa-fixed")) {
                ...
                $(".top-fa").css('paddingRight', '');
            }
        }
        $menu.css("top", (floatNumber + top));
    ...
    $('#growModal,#search-modal').on('show.bs.modal', function () {
        $('.top-fa-fixed').css('paddingRight', 213);
    });

    $('#growModal,#search-modal').on('hide.bs.modal', function () {
        $('.top-fa-fixed').css('paddingRight', 196);
    });
});
