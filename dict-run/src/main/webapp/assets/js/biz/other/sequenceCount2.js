require(['jquery', 'bootstrap', 'underscore', 'organTreeSelector', 'utils', 'messenger', 'selectItem',
    'jquery-mCustomScrollBar'], function ($) {
    ...
    window.countObj = {
        ...
        setLeftHTML: function (elemId) {
            ...
            if (_left.length > 0) {
                ...
                var html = '<span class="arrow_btn"></span><ul>';
        ...
        getSubDataCount: function (objs) {
            ...
            if (!_.isEmpty(objs)) {
                ...
                if (!_.isNull(objs)) count += parseInt(objs.empCount);
            }
            return count;
        },
        // 初始化序列页面
        ...
        initSubHTML: function (keyId) {
            ...
            if (!_.isUndefined(subObj) && _element.length > 0) {
                ...
                var arrayHtml = [
                    ...
                    '<span class="lbl"></span></label></span><div class="mCustomScrollbar">',
                    self.setSubHTML(subObj), '</div></div>'].join("");

                _element.html(arrayHtml);
                _element.find('.mCustomScrollbar').mCustomScrollbar({});
                $('.list .cate').each(function () {
                    $(this).removeClass('bgbbd7ea');
                });
    ...
    var navObj = {
        ...
        createNavSequence: function (firstObj) {
            ...
            self.createNavSubSeq(firstObj.id, false);
        },
