require(['jquery', 'echarts', 'zrender/shape/Text', 'echarts/chart/bar', 'echarts/chart/line', 'echarts/chart/pie',
    'bootstrap', 'jgGrid', 'underscore', 'timeLine2', 'utils', 'messenger'], function ($, echarts, TextShape) {
    ...
    var perBenefitOjb = {
        ...
        render: function () {
            ...
            //人均效益
            Tc.renderTopDigitalContrast($('#benefit'), Tc.formatFloat(lastMonth.benefitValue), Tc.formatFloat(twoMonthesAgo.benefitValue));

            //营业利润
            Tc.renderTopDigitalContrast($('#profit'), Tc.formatFloat(lastMonth.profit), Tc.formatFloat(twoMonthesAgo.profit));

            //等效全职员工数
            Tc.renderTopDigitalContrast($('#employeenumber'), Tc.formatFloat(lastMonth.eqEmpNum), Tc.formatFloat(twoMonthesAgo.eqEmpNum));
    ...
    var orgBenefitObj = {
        ...
        optionGrid: {
            ...
            colModel: [
                ...
                {name: 'benefitValue', width: 70, sortable: false, align: 'center'},
                {
                    name: 'profit', width: 90, sortable: false, align: 'center',
                    formatter: function (cellvalue) {
                        return Tc.formatDigital(Tc.formatFloat(cellvalue));
                    }
                },
                {
                    name: 'salesAmount', width: 70, sortable: false, align: 'center',
                    formatter: function (cellvalue) {
                        return Tc.formatDigital(Tc.formatFloat(cellvalue));
                    }
                },
                {
                    name: 'profitRate', width: 80, fixed: true, sortable: false, align: 'center',
...
//            var tpl3 = self.getRecentRateTpl(salesAmount.toFixed(0), preSalesAmountRate, '销售总额');
            ...
            var tpl1 = self.getRecentRateTpl(Tc.formatDigital(parseInt(benefit)), preBenfitRate, '人均效益');
            var tpl2 = self.getRecentRateTpl(Tc.formatDigital(parseInt(gainAmount)), preGainAmountRate, '利润总额');
            var tpl3 = self.getRecentRateTpl(Tc.formatDigital(salesAmount.toFixed(0)), preSalesAmountRate, '销售总额');
                $('#' + self.chartTextId).empty().append(tpl1 + tpl2 + tpl3);
        },
        getRecentRateTpl: function (num, percent, text) {
            return '<div class="row">' +
                '<div class="col-xs-12">' +
                '<div class="num pull-left">' + num + '万元</div>' +
                '</div>' +
                '<div class="col-xs-12 bottom-grey">近十二个月' + text + 
                '<b class="pull-right blue">' + percent + '</b>' +
                '</div>' +
                '</div>';
        },
        resizeGrid: function () {
    ...
    var fteObj = {
        option: {
        	 ...
        	 legend: {
                 data: [{name:'全职',icon:'bar'}, {name:'兼职',icon:'bar'},{name:'加班',icon:'bar'}],
                 y: 'bottom',
                 selectedMode: true
             },
            title: {
...
//            self.option.title.text = '合计 ' + Tc.formatNumber(total);
            self.initText('合计 ' + Tc.formatNumber(total));
