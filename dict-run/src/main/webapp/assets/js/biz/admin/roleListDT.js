...
//	        "order": [[1, 'asc']],// dt默认是第一列升序排列 这里第一列为序号列，所以设置为不排序，并把默认的排序列设置到后面
            ...
            "columnDefs":[{
            	...
            	"render": function(a,b,c,d){return '<input type="checkbox" name='+c.roleId+' value='+c.roleId+' />'}
					...
					"render": function (data, type, row, meta) {
						var fun ='<button type="button" class="btn btn-success btn-sm _fun" data-index='+row.roleId+'  ">配置功能  <span class="glyphicon glyphicon-th-list" /></button>'; 
						var org ='<button type="button" class="btn btn-info btn-sm _org" data-index='+row.roleId+'  ">配置数据  <span class=" glyphicon glyphicon-list-alt" /></button>'; 
						var edit ='<button type="button" class="btn btn-warning btn-sm _edit" data-id='+row.roleId+' data-key='+row.roleKey+' data-name='+row.roleName+' data-note='+row.note+' data-toggle="modal" data-target="#myModal" >修改  <span class="glyphicon glyphicon-wrench"></span></button>'; 
						var del ='<button type="button" class="btn btn-danger btn-sm _del" data-index='+row.roleId+' data-toggle="modal" data-target="#delModal">删除  <span class="glyphicon glyphicon-remove"></span></button>'; 
		    ...
		    "initComplete": function () {
		        $("#mytool").append('<button id="delBatch" type="button" class="btn btn-danger btn-sm">批量删除   <span class="glyphicon glyphicon-remove"></span></button>&nbsp');
		        $("#mytool").append('<button id="addRole" type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myModal">添加  <span class="glyphicon glyphicon-plus"></span></button>');
		    }
        }
