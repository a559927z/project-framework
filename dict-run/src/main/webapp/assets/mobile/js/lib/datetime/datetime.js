/**
 * Created by htpeng on 2016/8/25.
 */
!function($,IScroll){
...
DateTime.prototype = {
     ...
     *
     */
    presetType: ["ym","time", "date", "datetime", "diy"],
    /**
     *
     */
    dateLabels: {
    ...
    _initDateConfig: function () {
        ...
        var minYear = config.minDate.getFullYear();
		 var minMonth = config.minDate.getMonth()+1;
			
        var maxYear = config.maxDate.getFullYear();

		 var maxMonth = config.maxDate.getMonth()+1;
		...
		var mOffset=1;
		if(config.date.getFullYear()==minYear){
			mOffset=minMonth;
		}
        _this.m = {
            top: getTop(displayConfig.rm - mOffset ),
            value: displayConfig.m,
            ov: displayConfig.m,
            rv: displayConfig.rm,
            orv: displayConfig.rm,
            ...
            et: 12,
            list: [],
			s_map:fillArr(minMonth, 12),
			e_map:fillArr(1, maxMonth),
            map: fillArr(1, 12)
        };
        _this.d = {
            top: getTop(displayConfig.d - 1),
...
//        _this.itemList = ["y", "m", "d"];
    ...
    createListItem: function (map, value, key, list) {

        var _this = this;
        var index = getSelectedValueIndex(map, value);
    ...
    _renderHtml: function () {
        ...
        var createDateItem = function (config,map, value, key) {
            ...
            _this.createListItem(map, value, key, ul);
        ...
        _this.itemList.forEach(function (key) {
            ...
            var itemHtml;
            if ("m" == key) {
				var map=itemValue.map;
				if(_this.opts.date.getFullYear()==_this.opts.minDate.getFullYear()){
					map=itemValue.s_map;
				}else if(_this.opts.date.getFullYear()==_this.opts.maxDate.getFullYear()){
						map=itemValue.e_map;
				}
                itemHtml = createDateItem(itemValue,map, itemValue.rv, key);
            } else {
                itemHtml = createDateItem(itemValue,itemValue.map, itemValue.value, key);
            }
                ...
                scroll.on("scrollEnd", function () {
                    ...
                    if (_this[key].top != y) {
						...
						if(key=="y"){
							...
							_this.offsetM=0;
							var showM=time.m;
							if(time.y==_this.y.map[0]){
								 map=_this.m.s_map;
								 if(m<map[0]){
									m=map[0];
								 }
								 _this.offsetM=map[0]-1;
								 bool=false;
							}else if(time.y==_this.y.map[_this.y.map.length-1]){
									map=_this.m.e_map;

									if(m>map[map.length-1]){
									 m=map[map.length-1];
								 }
							}
							console.log(time);
							_this.refresh('m',map,m);
						}
						if(bool){
							time.m=m+_this.offsetM;
						}else{
							time.m=m;
						}
    ...
    },refresh:function(key,map,value){
		var _this=this;
		var ul=_this[key].ul;
		console.log(map);console.log(value);
//		  _this[key].xscroll.scrollTo(0, -40*(value-map[0]), 0, IScroll.utils.ease.circular);
		 _this[key].xscroll.scrollTo(0, 0, 0, IScroll.utils.ease.circular);
		$(ul).html("");
		 _this.createListItem(map, value, key, ul);
		 _this.bindEvent();
	},vaildYear:function(year){
			var map=_this.m.map;
			_this.offsetM=0;
			var showM=time.m;
			if(year==_this.y.map[0]){
				 map=_this.m.s_map;
				 if(m<map[0]){
					m=map[0];
				 }
