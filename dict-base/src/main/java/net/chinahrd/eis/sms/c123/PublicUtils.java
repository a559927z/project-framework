...
public class PublicUtils {
	...
	public static String joinArray(String[] mobileArray, String delim) {
		...
		String[] arrayOfString = mobileArray;
		int j = mobileArray.length;
		for (int i = 0; i < j; i++) {
			String m = arrayOfString[i];
	...
	public static List<SendResultBean> parseResult(String xml) {
		...
		List<SendResultBean> listItem = new ArrayList<SendResultBean>();
			...
			if (root != null) {
				...
				if (t.getResult() > 0) {
					...
					for (Element e : listResult) {
						...
						t.setUnitPrice(parseDouble(
								e.getAttributeValue("price"), 0.0D));

						int nRemain = 0;
						double dRemain = parseDouble(
								e.getAttributeValue("remain"), 0.0D);
						if (t.getUnitPrice() > 0.0D) {
							nRemain = (int) Math.round(dRemain
									/ t.getUnitPrice());
						}
						t.setRemain(nRemain);
						listItem.add(t);
			...
			if (root != null) {
				...
				if (t.getResult() > 0) {
					...
					if (item != null) {
						...
						int nRemain = 0;
						double dRemain = parseDouble(
								item.getAttributeValue("remain"), 0.0D);
	...
	public static List<SendStateBean> parseSendState(String xml) {
		...
		SAXBuilder builder = new SAXBuilder();
		try {
			List<SendStateBean> listSendState = new ArrayList<SendStateBean>();
			Document doc = builder.build(source);
			Element root = doc.getRootElement();
			if (root != null) {
				Attribute result = root.getAttribute("result");
				if (result != null) {
					if (parseInt(result.getValue(), -102) > 0) {
						...
						for (Element item : listItem) {
							...
							t.setMsgId(parseLong(
									item.getAttributeValue("msgid"), 0L));
	...
	public static List<ReplyBean> parseReply(String xml) {
		...
		SAXBuilder builder = new SAXBuilder();
		try {
			List<ReplyBean> listReply = new ArrayList<ReplyBean>();
			Document doc = builder.build(source);
			Element root = doc.getRootElement();
			if (root != null) {
				Attribute result = root.getAttribute("result");
				if (result != null) {
					if (parseInt(result.getValue(), -102) > 0) {
						...
						for (Element item : listItem) {
							...
							t.setMsgId(parseLong(
									item.getAttributeValue("msgid"), 0L));
							long lTime = parseLong(
									item.getAttributeValue("time"), 0L);
							if (lTime > 0L) {
